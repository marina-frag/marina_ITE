...
df['threshold'] = df['run'].str.extract(r'th(\d+)_').astype(int)
...
agg = (
    df
    .groupby(['threshold','split'])[metrics]
    .agg(['mean','sem'])
    .reset_index()
)
...
for m in metrics:
    ...
    out_path = os.path.join(out_dir, f"{m.lower().replace(' ','_')}_vs_threshold.png")
