...
agg = (
    df
    .groupby(['output_size','split'])[metrics]
    .agg(['mean','sem'])
    .reset_index()
)
...
for m in metrics:
    ...
    out_path = os.path.join(out_dir, f"{m.lower().replace(' ','_')}_vs_output_size.png")
